...
//---------------------------Chapter 2 : accessing the webcam ----------------------------------//

int main(){

    //first like image we need to make a video object, but instaed of path we will provide an id

    VideoCapture vdo(0); //if you only have one camera, thats the camera id 0. if there are more than one, then flipping through 0 
    //to number of total cameras-1 can help with camera id
    ...
    while(true){
        ...
        imshow("Gossip", img); //now one by one we will read frame and show it here on imshow,
        //note we are not using waitKey(0) since we dont want to pause just play the whole video at one go
        waitKey(1); //the delay here should be 1, just to make sure we are not too slow
    }
